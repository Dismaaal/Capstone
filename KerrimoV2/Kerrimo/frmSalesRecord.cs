    ...
    public partial class frmSalesRecord : Form
    {
        ...
        public void FillCombo()
        {
            ...
            try
            {
                ...
                adp.SelectCommand = new SqlCommand("SELECT distinct Name FROM PO,tblUserData where PO.[Employee Id]=tblUserData.[Employee Id]", con);
                ds = new DataSet("ds");
                adp.Fill(ds);
                dTable = ds.Tables[0];
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM(OrderNo) as [Order No],RTRIM(OrderDate) as [Order Date],RTRIM(PO.[EMPLOYEE ID]) as [Customer ID],RTRIM(Name) as [Customer Name],RTRIM(SubTotal) as [SubTotal],RTRIM(VATPer) as [Vat+ST %],RTRIM(VATAmount) as [VAT+ST Amount],RTRIM(DiscountPer) as [Discount %],RTRIM(DiscountAmount) as [Discount Amount],RTRIM(GrandTotal) as [Grand Total],RTRIM(TotalPayment) as [Total Payment],RTRIM(PaymentDue) as [Payment Due],RTRIM(Status) as [Status],Remarks from PO,tblUserData where PO.[EMPLOYEE ID]=tblUserData.[EMPLOYEE ID] and OrderDate between @d1 and @d2 order by OrderDate desc", con);
        ...
        private void cmbCustomerName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM(OrderNo) as [Order No],RTRIM(OrderDate) as [Order Date],RTRIM(PO.[EMPLOYEE ID]) as [Customer ID],RTRIM(Name) as [Customer Name],RTRIM(SubTotal) as [SubTotal],RTRIM(VATPer) as [Vat+ST %],RTRIM(VATAmount) as [VAT+ST Amount],RTRIM(DiscountPer) as [Discount %],RTRIM(DiscountAmount) as [Discount Amount],RTRIM(GrandTotal) as [Grand Total],RTRIM(TotalPayment) as [Total Payment],RTRIM(PaymentDue) as [Payment Due],RTRIM(Status) as [Status],Remarks from PO,tblUserData where PO.[EMPLOYEE ID]=tblUserData.[EMPLOYEE ID] and name='" + cmbCustomerName.Text.Trim() + "' order by Name,OrderDate", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "PO");
                myDA.Fill(myDataSet, "tblUserData");
        ...
        private void DataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                this.Hide();
                frmAdminPO frmSales = new frmAdminPO();
