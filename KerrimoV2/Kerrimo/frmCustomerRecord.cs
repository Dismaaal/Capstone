    ...
    public partial class frmCustomerRecord : Form
    {
        ...
        public void GetData()
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM([EMPLOYEE ID])as [Customer ID],RTRIM(Name) as [Customer Name],RTRIM([BRANCH]) as [Address],RTRIM(Contact) as [Contact No.], (email) as [Email] from tblUserData where [Priviledge Level] = 2 order by Name", con);
        ...
        private void frmCustomerRecord_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Hide();
            frmAdminPO frm = new frmAdminPO();
            frm.label6.Text = label1.Text;
        ...
        private void txtCustomers_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM([EMPLOYEE ID])as [Customer ID],RTRIM(Name) as [Customer Name],RTRIM(branch) as [Address],RTRIM(Contact) as [Contact No.], (email) as [Email] from tblUserData where Name  like '" + txtCustomers.Text + "%' and [Priviledge Level] = 2 order by Name", con);
